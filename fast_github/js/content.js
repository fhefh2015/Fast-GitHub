...
function addCloneButton() {
  const template = `<span class="d-flex">
      ...
      <div class="get-repo-modal dropdown-menu dropdown-menu-sw pb-0 js-toggler-container js-get-repo-modal p-3" style="width:352px;">
          <div class="get-repo-modal-options">
              ...
              <div class="mt-2 d-flex" style="text-align:center;">
                  <a class="flex-1 btn btn-outline get-repo-btn " rel="nofollow" href="${cf_url}/https://github.com/${github_auth_name}/${git_name}/archive/master.zip">
                  加速下载ZIP
                  </a>
              </div>
          </div>
...
function addReleaseButton() {
  ...
  releaseElems.forEach(function (elem) {
    const blockElems = elem.querySelector('a')
  ...
  const zipedElems = document.querySelectorAll(".release-entry .commit ul .d-inline-block");
  zipedElems.forEach(function (elem, index) {
    if (index > 1) {
      const zipElem = elem.querySelector("a");
    ...
    `;
      ...
      elem.appendChild(zip_frag);
    }
  });
  // for (let elem of releaseElems) {
